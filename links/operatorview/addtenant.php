...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(isset($_POST['addTen']))
  {
    ...
    if($stmt = $mysqli->prepare($occupantupdate)){
      ...
      if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("i", $param_tenantID);
            // Set parameters
            $param_tenantID = $haha;
            $stmt->execute();
            // Attempt to execute the prepared statement
        ...
        $noofocc1 = 0;
        $temlocation = "";
        $sqld1 = "SELECT number from room where roomnumber =? location=?";
        if($stmt = $mysqli->prepare($sqld1)){
          $stmt->bind_param("is", $roomnumber1, $temlocation);
            $roomnumber1 = $paramRoomnumberTenant;
            $temlocation = $_POST['Tenantlocation'];
            $stmt->execute();
            $result1 = $stmt->get_result();
             $roomAs1 = $result1->fetch_assoc();
             $noofocc1 = $roomAs1['number'];
            }
            $stmt->close();
        $sqld2 = "UPDATE room SET number=?  where roomnumber=? location=?";
        if($stmt = $mysqli->prepare($sqld2)){
          $stmt->bind_param("iis",$param_newOcc1, $roomnumber1, $temlocation);
          $param_newOcc1 = $noofocc1-1;
            $roomnumber1 = $paramRoomnumberTenant;
            $temlocation = $_POST['tenantlocation'];
            $stmt->execute();
            if($stmt->execute()){
